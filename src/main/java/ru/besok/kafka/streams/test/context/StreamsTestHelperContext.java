...
import ru.besok.kafka.streams.test.mock.MockKafkaAvroSerde;
...
public class StreamsTestHelperContext implements AutoCloseable {
  ...
  protected <T extends SpecificRecord> MockKafkaAvroSerde<T> serde(Class<T> clzz, boolean isKey) {
    MockKafkaAvroSerde<T> serde = new MockKafkaAvroSerde<>();
